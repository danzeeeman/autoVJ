...
void ProcessFFT::setup(bool useExternal){
    
    scaleFactor = 10000;
    numFFTbins = 16384;
    
    if(!useExternal){
        fft.setup(numFFTbins); //default
...
vector<float> ProcessFFT::getAudio(){
    return fft.getAudio();
}
...
void ProcessFFT::update(vector<float>& bins){
    calculateFFT(bins, FFTpercentage, bins.size());
    if(saveHistory){
        if (graphHigh.size()>graphMaxSize) {
            graphHigh.erase(graphHigh.begin(), graphHigh.begin()+1); 
            graphMid.erase(graphMid.begin(), graphMid.begin()+1);
...
float ProcessFFT::getIntensityAtFrequency(float _freq){
   ...
   // fft.getBins()[fft.getBins().size()];
    ...
    int whichBin;
    whichBin = fmodf(ofMap(_freq, 0, 22100, 0,bins.size(), true), bins.size()); //An approximation...
    
    float normalizedFreq;
    normalizedFreq = ofMap(bins[whichBin]*scaleFactor, 0, avgMaxSoundOverTime, 0, 1,true); //the scalefactor is just a scaling factor to have easier numbers to look at
